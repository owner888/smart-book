...
用户输入问题
     ...
     ▼
┌─────────────┐
│  chat.html  │  浏览器前端 (Layui + Marked.js)
│             │
│  fetch()    │──────────────────────┐
└─────────────┘                      │
                                     │ POST /api/stream/ask
                                     ▼
                            ┌─────────────────┐
                            │   Workerman     │  HTTP Server (Port 8088)
                            │                 │
                            │  handleStream   │
                            │     Ask()       │
                            └────────┬────────┘
        ...
        ▼                            ▼                            ▼
┌──────────────┐           ┌──────────────┐            ┌──────────────┐
│ Embedding    │           │ VectorStore  │            │ GeminiClient │
│ Client       │           │              │            │              │
│              │           │ hybridSearch │            │ chatStream() │
│ embedQuery() │           │ (关键词+向量) │            │              │
└──────┬───────┘           └──────┬───────┘            └──────┬───────┘
       │                          │                           │
       │ 生成查询向量              │ 检索相关文档               │ 流式生成回答
       ▼                          ▼                           ▼
┌──────────────┐           ┌──────────────┐            ┌──────────────┐
│   Gemini     │           │  书籍索引     │            │   Gemini     │
│ Embedding    │           │  (JSON缓存)   │            │   2.5 Flash  │
│    API       │           │              │            │     API      │
└──────────────┘           └──────────────┘            └──────────────┘
  ...
  Gemini API ──(SSE)──▶ curl 回调 ──(立即)──▶ sendSSE() ──(SSE)──▶ 浏览器
      │                    │                      │                  │
      │                    │                      │                  │
   token               onChunk()           event: content      实时渲染
   by token              回调                data: ...         Markdown
```
...
┌──────────────┐    SSE     ┌──────────────┐   立即回调   ┌──────────────┐
│              │  ────────▶ │    curl      │  ─────────▶ │   sendSSE    │
│  Gemini API  │   token    │ WRITEFUNCTION│   onChunk   │  (Workerman) │
│              │  by token  │    回调      │             │              │
└──────────────┘            └──────────────┘             └──────┬───────┘
                                                               │
                                                          SSE  │
...
**特点**：端到端流式传输，Gemini 每生成一个 token 立即传递到浏览器显示！
