...
// ===================================
// MCP SSE Server 请求处理（端口 8090）
// 支持 Server-Sent Events 实时通知
// ===================================

// MCP SSE Server 实例（延迟初始化）
$mcpSseServer = null;

/**
 * 获取 MCP SSE Server 实例
 */
function getMCPSSEServer(): \SmartBook\MCP\StreamableHttpSSEServer
{
    global $mcpSseServer;
    
    if ($mcpSseServer === null) {
        $booksDir = dirname(__DIR__, 2) . '/books';
        $mcpSseServer = new \SmartBook\MCP\StreamableHttpSSEServer($booksDir);
    }
 ...
 * 
 * 协议特点：
 * - GET /sse: 建立 SSE 连接，接收实时通知
 * - POST /message: 发送 JSON-RPC 消息
 * - 支持进度通知、资源变更通知等
 */
function handleMCPSSERequest(TcpConnection $connection, Request $request): void
{
    $server = getMCPSSEServer();
    $server->handleRequest($connection, $request);
}
